...
/**
 *	\file include/qpOASES/LCQProblemB.hpp
 *	\author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches
 *	\version 3.2
 *	\date 2007-2017
 *
 *	Declaration of the LCQProblemB class which is able to use the newly
...
#ifndef QPOASES_LCQPROBLEMB_HPP
#define QPOASES_LCQPROBLEMB_HPP
...
class LCQProblemB
{
	...
	public:
		...
		LCQProblemB(	int_t _nV,								/**< Number of variables. */
		...
		LCQProblemB(	const LCQProblemB& rhs	/**< Rhs object. */
					);
		...
		virtual LCQProblemB& operator=(	const LCQProblemB& rhs	/**< Rhs object. */
								);


		/** Clears all data structures of LCQProblemB except for QP data.
		...
		returnValue init(	SymmetricMatrix *_H,					/**< Hessian matrix (a shallow copy is made). */
                            ...
                            SymmetricMatrix *_C,                   /**< Complementarity matrix. */
		...
		returnValue init(	const real_t* const _H, 				/**< Hessian matrix (a shallow copy is made). \n
							...
							const real_t* const _ub,				/**< Upper bounds (on variables). \n
																		 If no upper bounds exist, a NULL pointer can be passed. */
                            const real_t* const _C,                 /**< Complementarity matrix. */
							int_t& nWSR, 							/**< Input: Maximum number of working set recalculations when using initial homotopy. \n
																		 Output: Number of performed working set recalculations. */
				 			real_t* const cputime = 0,				/**< Input: Maximum CPU time allowed for QP initialisation. \n
																		 Output: CPU time spent for QP initialisation (if pointer passed). */
		...
		returnValue init(	const char* const H_file,				/**< Name of file where Hessian matrix is stored. \n
							...
							const char* const ub_file,				/**< Name of file where upper bound vector. \n
																		 If no upper bounds exist, a NULL pointer can be passed. */
							const char* const C_file,				/**< Name of file where complementarity matrix is stored. */
							int_t& nWSR, 							/**< Input: Maximum number of working set recalculations when using initial homotopy. \n
																		 Output: Number of performed working set recalculations. */
				 			real_t* const cputime = 0,				/**< Input: Maximum CPU time allowed for QP initialisation. \n
																		 Output: CPU time spent for QP initialisation (if pointer passed). */
		 ...
		 *
		 *  Note: This function internally calls solveLCQP/solveRegularisedLCQP
		 ...
		 *	\return BT_TRUE:  LCQProblemB initialised \n
		 			BT_FALSE: LCQProblemB not initialised */
		inline BooleanType isInitialised( ) const;

		/** Returns if the QP has been solved.
		 *	\return BT_TRUE:  LCQProblemB solved \n
		 			BT_FALSE: LCQProblemB not solved */
		...
		/** Performs Armijo sufficient decrease along delta_x and updates t.
		 *  \return SUCCSESSFUL_RETURN. */
		returnValue ensureSufficientDecrease( 	const real_t* const delta_x,
	...
	protected:
		 ...
		 *  \return SUCCESSFUL_RETURN */
		returnValue copy(	const LCQProblemB& rhs	/**< Rhs object. */
							);
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_INVALID_ARGUMENTS \n
					RET_NO_HESSIAN_SPECIFIED */
		returnValue setupLCQPdataFromFile(	const char* const H_file, 				/**< Name of file where Hessian matrix, of neighbouring QP to be solved, is stored. \n
																     					 If Hessian matrix is trivial,a NULL pointer can be passed. */
		...
		/** Sets the dense complementarity matrix. */
		inline returnValue setC(SymmetricMatrix* _C_new);

		/** Sets the dense complementarity matrix. */
		inline returnValue setC(const real_t* const _C_new);
		...
		real_t* getNLPStationarity(	const real_t* const x_eval,
									const real_t penVal,
									const real_t* const g_original
									);

		/** Returns the merit function. */
		real_t getMeritValue(	const real_t* const x_eval,
								const real_t penVal,
								const real_t* const g_original
								);

		/** Returns the gradient of the merit function. */
		real_t* getGradientMeritValue(	const real_t* const x_eval,
										const real_t penVal,
										const real_t* const g_original
										);

		/** Returns the directional derivative of the merit function along delta_x. */
		real_t getDMeritValue(	const real_t* const x_eval,
	 ...
	 */
	private:
		/** Solves a LCQProblemB whose QP data is assumed to be stored in the member variables.
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_INIT_FAILED_UNBOUNDEDNESS \n
					RET_MAX_NWSR_REACHED */
		returnValue solveInitialLCQP(	const real_t* const xOpt,			/**< Optimal primal solution vector.*/
                                        const real_t* const yOpt,			/**< Optimal dual solution vector. */
		...
		/** Solves an initialised LCQProblemB using online active set strategy.
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_HOTSTART_STOPPED_INFEASIBILITY \n
					RET_HOTSTART_STOPPED_UNBOUNDEDNESS */
		returnValue solveLCQP(	const real_t* const g_new,			/**< Gradient of neighbouring QP to be solved. */
								const real_t* const lb_new,			/**< Lower bounds of neighbouring QP to be solved. \n
													 					 If no lower bounds exist, a NULL pointer can be passed. */
								const real_t* const ub_new,			/**< Upper bounds of neighbouring QP to be solved. \n
													 					 If no upper bounds exist, a NULL pointer can be passed. */
								...
								int_t  nWSRperformed = 0,			/**< Number of working set recalculations already performed to solve
																		 this QP within previous solveLCQP() calls. This number is
																		 always zero, except for successive calls from solveRegularisedLCQP()
		 ...
		 *	\return SUCCESSFUL_RETURN \n
					...
					RET_HOTSTART_STOPPED_INFEASIBILITY \n
					RET_HOTSTART_STOPPED_UNBOUNDEDNESS */
		returnValue solveRegularisedLCQP(	const real_t* const g_new,			/**< Gradient of neighbouring QP to be solved. */
                                            const real_t* const lb_new,			/**< Lower bounds of neighbouring QP to be solved. \n
                                                                                    If no lower bounds exist, a NULL pointer can be passed. */
                                            const real_t* const ub_new,			/**< Upper bounds of neighbouring QP to be solved. \n
                                                                                    If no upper bounds exist, a NULL pointer can be passed. */
                                            ...
                                            real_t* const cputime,				/**< Input: Maximum CPU time allowed for QP solution. \n
                                                                                    Output: CPU time spent for QP solution (or to perform nWSR iterations). */
                                            int_t nWSRperformed = 0,			/**< Number of working set recalculations already performed to solve
                                                                                    this QP within previous solveRegularisedLCQP() calls. This number is
	...
	protected:
		...
		/** Minimize merit function along current step and return optimal length. */
		real_t minimizeMeritFun(const real_t* const g_new, 		/**< Linear objective term. */
								real_t penVal					/**< Current penalty value. */
								);	

		returnValue performPenaltyGradientUpdate(	const real_t* const g_original, 
													real_t* const g_update,
													const real_t penVal,
													ComplementarityStatus& comp_status
													);

		/** Evaluates the the penalty function.
		 * \return Evaluated penalty function. */
		real_t PenFun(const real_t* const x);

		/** Evaluates the gradient of the penalty function.
		 * \return Evaluated gradient of penalty function. */
		real_t* GradPenFun(const real_t* const x, const real_t penVal);

		/** Computes the sum of two vectors.
		 * \return Sum of two vectors. */
		real_t* AddGradients(const real_t* const g, const real_t* const d);

		/** Computes the sum of alpha*a + beta*b.
		 * \return alpha*a + beta*b. */
		real_t* AddVectors(const real_t alpha, const real_t* const a, const real_t beta, const real_t* const b, int_t len) const;
	private:
		...
		returnValue performStep(	const real_t* const delta_g,	/**< Step direction of gradient. */
									...
									const real_t penVal,			/**< Current penalty value. */
									const real_t* const g_original,	/**< Original objectives linear term. */
									int_t& BC_idx, 					/**< Output: Index of blocking constraint. */
		...
		returnValue printIteration(	int_t iter,							/**< Number of current iteration. */
									...
									real_t penaltyValue,				/**< Current penalty value. */
									BooleanType isFirstCall = BT_TRUE	/**< Indicating whether this is the first call for current QP. */
									);
	...
	protected:
		...
		BooleanType freeC;			/**< Flag indicating whether the Hessian matrix needs to be de-allocated. */
		SymmetricMatrix* C;			/**< Complementarity matrix. */

		real_t* g_lin;				/**< Current linearization of the penalty function. */
		...
		ComplementarityStatus comp_status;	/**< Current status of complementarity. */
		...
		real_t *xk_lin;				/**< Last complementarity linearization point. */
		real_t *pk;					/**< Last step. */
		...
		real_t old_merit_value;		/**< Keep track of old merit function value. */
		real_t* old_pen_lin;		/**< Keep track of old penalty linearization. */
...
#include <qpOASES/LCQProblemB.ipp>

#endif	/* QPOASES_LCQPROBLEMB_HPP */
