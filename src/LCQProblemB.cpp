...
/**
 *	\file src/LCQProblemB.cpp
 *	\author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches
 *	\version 3.2
 *	\date 2007-2017
 *
 *	Implementation of the LCQProblemB class which is able to use the newly
 ...
 *	Q P r o b l e m B
 */
LCQProblemB::LCQProblemB( int_t _nV, HessianType _hessianType, BooleanType allocDenseMats )
{
	int_t i;
	...
	pk = new real_t[_nV];
	for (i = 0; i < _nV; i++) pk[i] = 0.0;

	xk_lin = new real_t[_nV];
	for (i = 0; i < _nV; i++) xk_lin[i] = 0.0;
 ...
 *	Q P r o b l e m B
 */
LCQProblemB::LCQProblemB( const LCQProblemB& rhs )
{
	freeHessian = BT_FALSE;
	H = 0;
 ...
 *	o p e r a t o r =
 */
LCQProblemB& LCQProblemB::operator=( const LCQProblemB& rhs )
{
	if ( this != &rhs )
	{
		clear( );
 ...
 *	i n i t
 */
returnValue LCQProblemB::init( 	SymmetricMatrix *_H, const real_t* const _g,
								...
								)
{
	...
	/* 2) Setup QP data. */
	if ( setupLCQPdata( _H,_g,_lb,_ub, _C ) != SUCCESSFUL_RETURN )
		return THROWERROR( RET_INVALID_ARGUMENTS );

	/* 3) Call to main initialisation routine. */
	return solveInitialLCQP( xOpt,yOpt,guessedBounds,_R, nWSR,cputime );
}
 ...
 *	i n i t
 */
returnValue LCQProblemB::init( 	const real_t* const _H, const real_t* const _g,
								const real_t* const _lb, const real_t* const _ub,
                                const real_t* const _C,
								int_t& nWSR, real_t* const cputime,
								const real_t* const xOpt, const real_t* const yOpt,
								const Bounds* const guessedBounds,
								const real_t* const _R
 ...
 *	i n i t
 */
returnValue LCQProblemB::init( 	const char* const H_file, const char* const g_file,
								const char* const lb_file, const char* const ub_file,
								const char* const C_file,
								int_t& nWSR, real_t* const cputime,
								const real_t* const xOpt, const real_t* const yOpt,
								const Bounds* const guessedBounds,
								...
								)
{
	...
	/* 2) Setup LCQP data from files. */
	if ( setupLCQPdataFromFile( H_file,g_file,lb_file,ub_file,C_file ) != SUCCESSFUL_RETURN )
		return THROWERROR( RET_UNABLE_TO_READ_FILE );

	if ( R_file == 0 )
	{
		/* 3) Call to main initialisation routine. */
		return solveInitialLCQP( xOpt,yOpt,guessedBounds,0, nWSR,cputime );
	}
	else
	{
		...
		return solveInitialLCQP( xOpt,yOpt,guessedBounds,R, nWSR,cputime );
	}
}
 ...
 *	h o t s t a r t
 */
returnValue LCQProblemB::hotstart(	const real_t* const g_new,
									...
									)
{
	...
	{
		/* Automatically call standard solveLCQP if regularisation is not active. */
		returnvalue = solveRegularisedLCQP(	g_new,lb_new,ub_new,
											nWSR,cputime,0,
											isFirstCall
											);
	}
	else
	{
		...
		for ( ;; )
		{
			...
			/* Automatically call standard solveLCQP if regularisation is not active. */
			returnvalue = solveRegularisedLCQP(	g_new,lb_new_far,ub_new_far,
 ...
 *	h o t s t a r t
 */
returnValue LCQProblemB::hotstart(	const char* const g_file,
									const char* const lb_file, const char* const ub_file,
									int_t& nWSR, real_t* const cputime,
									const Bounds* const guessedBounds
									)
 ...
 */
returnValue LCQProblemB::getWorkingSet( real_t* workingSet )
{
 ...
 */
returnValue LCQProblemB::getWorkingSetBounds( real_t* workingSetB )
{
	int_t i;
	int_t nV = this->getNV();
 ...
 */
returnValue LCQProblemB::getWorkingSetConstraints( real_t* workingSetC )
{
	if ( workingSetC == 0 )
		return THROWERROR( RET_INVALID_ARGUMENTS );
	else
 ...
 *	g e t O b j V a l
 */
real_t LCQProblemB::getObjVal( ) const
{
	real_t objVal;
 ...
 *	g e t O b j V a l
 */
real_t LCQProblemB::getObjVal( const real_t* const _x ) const
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::getPrimalSolution( real_t* const xOpt ) const
{
	int_t i;
 ...
 */
returnValue LCQProblemB::getDualSolution( real_t* const yOpt ) const
{
	int_t i;
 ...
 */
returnValue LCQProblemB::setPrintLevel( PrintLevel _printLevel )
{
 ...
 */
returnValue LCQProblemB::printProperties( )
{
	#ifndef __SUPPRESSANYOUTPUT__
...
returnValue LCQProblemB::printOptions( ) const
{
	return options.print( );
}

returnValue LCQProblemB::ensureSufficientDecrease( 	const real_t* const delta_x,
													...
													const real_t penVal ) 
{
	...
	real_t* x_new = new real_t[nV];

	real_t rhs_const1 = getMeritValue(x, penVal, g_original);
	real_t rhs_const2 = getDMeritValue(x, delta_x, penVal, g_original);

	while (true) {
		// Compute new step
		for (int i = 0; i < nV; i++) {
			x_new[i] = x[i] + t*delta_x[i];
		}

		// Evaluate Armijo LHS
		real_t lhs = getMeritValue(x_new, penVal, g_original);

		// Evaluate Armijo RHS
		real_t rhs = rhs_const1 + gamma*t*rhs_const2;
 ...
 *	c o p y
 */
returnValue LCQProblemB::copy(	const LCQProblemB& rhs
								)
{
	...
	if ( freeC == BT_TRUE )
		C = (SymmetricMatrix *)(rhs.C->duplicateSym());
	else
		C = rhs.C;
 ...
 */
returnValue LCQProblemB::determineHessianType( )
{
	int_t i;
	int_t nV = getNV( );
	real_t curDiag;
 ...
 */
returnValue LCQProblemB::setupSubjectToType( )
{
 ...
 *	s e t u p S u b j e c t T o T y p e
 */
returnValue LCQProblemB::setupSubjectToType( const real_t* const lb_new, const real_t* const ub_new )
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::computeCholesky( )
{
	int_t i, j;
	int_t nV  = getNV( );
	int_t nFR = getNFR( );
 ...
 */
returnValue LCQProblemB::setupInitialCholesky( )
{
 ...
 */
returnValue LCQProblemB::obtainAuxiliaryWorkingSet(	const real_t* const xOpt, const real_t* const yOpt,
 ...
 *	b a c k s o l v e R
 */
returnValue LCQProblemB::backsolveR(	const real_t* const b, BooleanType transposed,
									real_t* const a
									) const
{
 ...
 */
returnValue LCQProblemB::determineDataShift(	const real_t* const g_new, const real_t* const lb_new, const real_t* const ub_new,
...
/*
 *	s e t u p L C Q P d a t a
 */
returnValue LCQProblemB::setupLCQPdata(	SymmetricMatrix *_H, const real_t* const _g,
									    const real_t* const _lb, const real_t* const _ub,
                                        SymmetricMatrix *_C
									    )
{
 ...
 */
returnValue LCQProblemB::setupLCQPdata(	const real_t* const _H, const real_t* const _g,
								    	const real_t* const _lb, const real_t* const _ub,
                                        const real_t* const _C
									    )
{
 ...
 */
returnValue LCQProblemB::setupLCQPdataFromFile(	const char* const H_file, const char* const g_file,
												const char* const lb_file, const char* const ub_file,
												const char* const C_file
												)
{
	...
	/* 5) Load complementarity matrix from file. */
	if ( C_file != 0 )
	{
		real_t* _C = new real_t[nV * nV];
		returnvalue = readFromFile( _C, nV,nV, C_file );
		if ( returnvalue != SUCCESSFUL_RETURN )
		{
			delete[] _C;
			return THROWERROR( returnvalue );
	...
	{
		THROWERROR( RET_MISSING_COMPLEMENTARITY_MATRIX );
	}
 ...
 */
returnValue LCQProblemB::loadQPvectorsFromFile(	const char* const g_file, const char* const lb_file, const char* const ub_file,
 ...
 */
returnValue LCQProblemB::setInfeasibilityFlag(	returnValue returnvalue,
 ...
 */
returnValue LCQProblemB::areBoundsConsistent(	const real_t* const lb_new, const real_t* const ub_new ) const
{
	if (lb_new && ub_new) {
 ...
 */
BooleanType LCQProblemB::isCPUtimeLimitExceeded(	const real_t* const cputime,
												real_t starttime,
												int_t nWSR
												) const
{
 ...
 */
returnValue LCQProblemB::regulariseHessian( )
{
 ...
 */
SymSparseMat* LCQProblemB::createDiagSparseMat( int_t n, real_t diagVal )
{
 ...
 */
returnValue LCQProblemB::performRatioTest(	int_t nIdx,
 ...
 */
real_t LCQProblemB::getRelativeHomotopyLength(	const real_t* const g_new, const real_t* const lb_new, const real_t* const ub_new
												)
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::performRamping( )
{
 ...
 */
returnValue LCQProblemB::updateFarBounds(	real_t curFarBound, int_t nRamp,
...
/*
 *  g e t N L P S t a t i o n a r i t y
 */
real_t* LCQProblemB::getNLPStationarity(const real_t* const x_eval,
										const real_t penVal,
										const real_t* const g_original)
{
	int_t nV = getNV();
	real_t* stat = getGradientMeritValue(x_eval, penVal, g_original);
...
/*
 *  g e t M e r i t V a l u e
 */
real_t LCQProblemB::getMeritValue(	const real_t* const x_eval,
									const real_t penVal,
									const real_t* const g_original
									)
{
	...
	// Copy objectives linear term
	real_t* grad = new real_t[nV];
	memcpy(grad, g_original, nV);

	// Evaluate gradient of objective
	H->times(1, 1.0, x_eval, nV, 1.0, grad, nV);

	return getObjVal(x_eval) + penVal*PenFun( x_eval );
}

/*
 *  g e t D M e r i t V a l u e
 */
real_t* LCQProblemB::getGradientMeritValue(	const real_t* const x_eval,
											const real_t penVal,
											const real_t* const g_original)
{
	int_t nV = getNV();
	...
	real_t* pen_grad = GradPenFun(x_eval, penVal);

	// Add together
	return AddGradients(grad, pen_grad);
}

/*
 *  g e t D M e r i t V a l u e
 */
real_t LCQProblemB::getDMeritValue(	const real_t* const x_eval,
									const real_t* const delta_x,
									const real_t penVal,
									const real_t* const g_original)
{
	int_t nV = getNV();
	...
	real_t* totalGradient = getGradientMeritValue(x_eval, penVal, g_original);
		
	// Compute the scalar product 
	real_t ret = 0.0;
	for (int i = 0; i < nV; i++) {
		ret += totalGradient[i]*delta_x[i];
	}
...
/*
 *  m i n i m i z e M e r i t F u n
 */
real_t LCQProblemB::minimizeMeritFun(	const real_t* const g_new, 		/**< Linear objective term. */
										real_t penVal					/**< Current penalty value. */
									) 
{
	int_t nV = getNV( );

	// 0) Always perform full step when step size is small! 
	//    Usually happens close to solutions only.
	if (getNorm(pk, nV) < options.terminationTolerance)
		return 1;
	
	// I) Build merit objective along pk
	// 1) Build quadratic term
	real_t qk = 0;
	real_t* qk_tmp1 = new real_t[nV];
	real_t* qk_tmp2 = new real_t[nV];

	// Hessian contribution
	H->times(1, 1, pk, nV, 0, qk_tmp1, nV);

	// Complementarity contribution
	qk_tmp2 = GradPenFun(pk, penVal);

	// 2) Build linear term
	real_t lk = 0;
	real_t* lk_tmp1 = new real_t[nV];
	real_t* lk_tmp2 = new real_t[nV];

	// Hessian contribution
	H->times(1, 1, xk_lin, nV, 0, lk_tmp1, nV);

	// Complementarity contribution
	lk_tmp2 = GradPenFun(xk_lin, penVal);
	
	// 3) Evaluate
	for (int i = 0; i < nV; i++) {
		qk += pk[i]*(qk_tmp1[i] + qk_tmp2[i]);
		lk += pk[i]*(lk_tmp1[i] + lk_tmp2[i] + g_new[i]);
	}

	// II) Handle the various cases
	// 1) Nonconvex: Solution is either 1 or 0
	if (qk <= 0)
	{
		// If merit at 1 <= merit at 0 return full step
		if (qk + lk <= 0)
			return 1;
		else
			return 0;
...
/*
 *   p e r f o r m P e n a l t y G r a d i e n t U p d a t e
 */
returnValue LCQProblemB::performPenaltyGradientUpdate( 	const real_t* const g_original, 
														real_t* const g_update,
														const real_t penVal,
														ComplementarityStatus& comp_status )
{
	// Compare merit value to old linearization
	real_t merit_val = getMeritValue(x, penVal, g_original);
	
	//if (merit_val < old_merit_value) {
		// Get new linearization
		g_lin = GradPenFun(x, penVal);

		// Store value in g_update
		for (int i = 0; i < getNV(); i++)
			g_update[i] = g_original[i] + g_lin[i];

		// Keep track of old linearization point and merit value
		old_pen_lin = x;
		old_merit_value = merit_val;

		// Remember that we've updated the gradient
		comp_status = GRADIENT_UPDATED;
	//}
...
*	P E N F U N
*/
real_t LCQProblemB::PenFun( const real_t* const x )
{
	int_t nV = getNV();

	real_t* grad_pen = GradPenFun(x, 1);
 ...
 *    G r a d P e n F u n
 */
real_t* LCQProblemB::GradPenFun(	const real_t* const x,
									const real_t penVal
								)
{
	int_t nV = getNV();

	real_t* ret = new real_t[nV];

	C->times(1, 1, x, nV, 0, ret, nV);
...
/*
 *  A d d G r a d i e n t s
 */
real_t* LCQProblemB::AddGradients(const real_t* const g,
 ...
 *  A d d V e c t o r s
 */
real_t* LCQProblemB::AddVectors(	const real_t alpha,
...
{
	real_t* ret = new real_t[len];
	for (int i = 0; i < len; i++) {
			ret[i] = alpha*a[i] + beta*b[i];
	}
 ...
 */
returnValue LCQProblemB::solveInitialLCQP(  const real_t* const xOpt, const real_t* const yOpt,
										    const Bounds* const guessedBounds,
										    const real_t* const _R,
										    int_t& nWSR, real_t* const cputime
										    )
{
	...
	/* Store initial guess as first linearization point. */
	if (xOpt != 0) 
		for (i = 0; i < nV; i++)
			xk_lin[i] = xOpt[i];
 ...
 *	s o l v e Q P
 */
returnValue LCQProblemB::solveLCQP(	const real_t* const g_new,
                                    const real_t* const lb_new, const real_t* const ub_new,
                                    ...
                                    )
{
    ...
    real_t complementarityValue;
	real_t nlpStationarityValue;

	old_merit_value = INFINITY;
	old_pen_lin = x;

    real_t penVal = options.initialComplementarityPenalty;
	...
	/* 4) Linearize complementarities. */
	real_t* g_update = new real_t[nV];

	g_update = AddGradients(g_new, GradPenFun(xk_lin, penVal));
	// for (int i = 0; i < nV; i++)
	//  	g_update[i] = g_new[i];
	...
	for( iter=nWSRperformed; iter<nWSR; ++iter )
	{
		...
		returnvalue = determineDataShift(	g_update,lb_new,ub_new,
		...
		homotopyLength = getRelativeHomotopyLength(g_update, lb_new, ub_new);

		if ( homotopyLength <= options.terminationTolerance)
		{
			// Get NLP stationarity value
			nlpStationarityValue = getNorm(getNLPStationarity(x, penVal, g_new), 1);
			
			if (nlpStationarityValue <= options.terminationTolerance) {
			
				// Get complementarity value
				complementarityValue = PenFun( x );

				if (complementarityValue <= options.complementarityTolerance)
				{
					status = QPS_SOLVED;
					comp_status = COMP_SATISFIED;

					THROWINFO( RET_OPTIMAL_SOLUTION_FOUND );

					if ( printIteration( iter,BC_idx,BC_status, homotopyLength, penVal, isFirstCall ) != SUCCESSFUL_RETURN )
				...
				penVal *= options.complementarityPenaltyUpdate;
				comp_status = COMP_GROWING;
			} 
			...
			// Update gradient at merit minimizer
			real_t alpha_opt = minimizeMeritFun(g_new, penVal);
			...
			if (alpha_opt == 0) 
			{
				...
				// Update linearization point and gradient
				for (int j = 0; j < nV; j++)
					xk_lin[j] += alpha_opt*pk[j];

				if (comp_status != COMP_GROWING)
					comp_status = GRADIENT_UPDATED;
			}
		...
		if ( returnvalue != SUCCESSFUL_RETURN && comp_status == COMP_UNSATISFIED)
		{
			delete[] delta_yFX; delete[] delta_xFX; delete[] delta_xFR;
			delete[] delta_ub; delete[] delta_lb; delete[] delta_g;
		...
		/* 7a) Possibly refactorise projected Hessian from scratch. */
		...
		/* 9) Output information of successful QP iteration. */
		status = QPS_HOMOTOPYQPSOLVED;
 ...
 */
returnValue LCQProblemB::solveRegularisedLCQP(	const real_t* const g_new,
                                                const real_t* const lb_new, const real_t* const ub_new,
                                                int_t& nWSR, real_t* const cputime, int_t nWSRperformed,
                                                BooleanType isFirstCall
                                                )
{
	...
	if ( usingRegularisation( ) == BT_FALSE )
		return solveLCQP( g_new,lb_new,ub_new, nWSR,cputime,nWSRperformed,isFirstCall );
	...
	if ( cputime == 0 )
	{
		returnvalue = solveLCQP( g_new,lb_new,ub_new, nWSR,0,nWSRperformed,isFirstCall );
	}
	else
	{
		cputime_cur = *cputime;
		returnvalue = solveLCQP( g_new,lb_new,ub_new, nWSR,&cputime_cur,nWSRperformed,isFirstCall );
	}
	nWSR_total     = nWSR;
	...
	for( step=0; step<options.numRegularisationSteps; ++step )
	{
		...
		{
			nWSR = nWSR_max;
			returnvalue = solveLCQP( gMod,lb_new,ub_new, nWSR,0,nWSR_total,isFirstCall );
		}
		else
		{
			nWSR = nWSR_max;
			cputime_cur = *cputime - cputime_total;
			returnvalue = solveLCQP( gMod,lb_new,ub_new, nWSR,&cputime_cur,nWSR_total,isFirstCall );
		}
 ...
 */
returnValue LCQProblemB::setupAuxiliaryWorkingSet( 	const Bounds* const auxiliaryBounds,
 ...
 */
returnValue LCQProblemB::setupAuxiliaryQPsolution(	const real_t* const xOpt, const real_t* const yOpt
													)
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::setupAuxiliaryQPgradient( )
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::setupAuxiliaryQPbounds( BooleanType useRelaxation )
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::setupAuxiliaryQP( const Bounds* const guessedBounds )
{
	int_t i;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblemB::determineStepDirection(	const real_t* const delta_g, const real_t* const delta_lb, const real_t* const delta_ub,
												BooleanType Delta_bB_isZero,
 ...
 */
returnValue LCQProblemB::performStep(	const real_t* const delta_g,
										...
										const real_t penVal, const real_t* const g_original,
 ...
 */
returnValue LCQProblemB::changeActiveSet( int_t BC_idx, SubjectToStatus BC_status )
{
	#ifndef __SUPPRESSANYOUTPUT__
	char messageString[MAX_STRING_LENGTH];
	#endif
 ...
 */
returnValue LCQProblemB::performDriftCorrection( )
{
	int_t i;
	int_t nV = getNV ();
 ...
 */
BooleanType LCQProblemB::shallRefactorise( const Bounds* const guessedBounds ) const
{
	int_t i;
	int_t nV = getNV( );
 ...
 *	a d d B o u n d
 */
returnValue LCQProblemB::addBound(	int_t number, SubjectToStatus B_status,
									BooleanType updateCholesky
									)
{
	int_t i, j;
	...
	/* Perform cholesky updates only if LCQProblemB has been initialised! */
 ...
 */
returnValue LCQProblemB::removeBound(	int_t number,
									BooleanType updateCholesky
									)
{
	int_t i;
 ...
 */
returnValue LCQProblemB::printIteration(int_t iter,
										int_t BC_idx,
										SubjectToStatus BC_status,
										real_t homotopyLength,
										real_t penaltyValue,
										...
										)
{
	...
	switch ( options.printLevel )
	{
		...
		case PL_MEDIUM:
			...
			{
				if (comp_status == COMP_GROWING)
					snprintf( info,17,"INC PEN %1.2e", penaltyValue );
				else if ( hessianType == HST_ZERO )
					snprintf( info,17,"   LP SOLVED     " );
				else
					snprintf( info,17,"   QP SOLVED     " );

				if ( isFirstCall == BT_TRUE )
					snprintf( myPrintfString,MAX_STRING_LENGTH,"   %5.1d   |   %1.6e   | %s |  %4.1d   \n", (int)iter,tau,info,(int)getNFX( ) );
				else
					snprintf( myPrintfString,MAX_STRING_LENGTH,"   %5.1d*  |   %1.6e   | %s |  %4.1d   \n", (int)iter,tau,info,(int)getNFX( ) );
				myPrintf( myPrintfString );
			}
