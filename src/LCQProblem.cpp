 ...
 *	L C Q P r o b l e m
 */
LCQProblem::LCQProblem( ) : LCQProblemB( )
{
 ...
 *	Q P r o b l e m
 */
LCQProblem::LCQProblem( int_t _nV, int_t _nC, HessianType _hessianType, BooleanType allocDenseMats )
	: LCQProblemB( _nV, _hessianType,allocDenseMats )
{
	int_t i;
 ...
 *	Q P r o b l e m
 */
LCQProblem::LCQProblem( const LCQProblem& rhs ) : LCQProblemB( rhs )
{
	freeConstraintMatrix = BT_FALSE;
	A = 0;
 ...
 *     o p e r a t o r =
 */
LCQProblem& LCQProblem::operator=( const LCQProblem& rhs )
{
       if ( this != &rhs )
       {
               clear( );
               LCQProblemB::operator=( rhs );
               copy( rhs );
       }
...
returnValue LCQProblem::reset( )
{
	...
	if ( LCQProblemB::reset( ) != SUCCESSFUL_RETURN )
 ...
 *	i n i t
 */
returnValue LCQProblem::init(	SymmetricMatrix *_H, const real_t* const _g, Matrix *_A,
								...
								)
{
	...
	/* 2) Setup QP data. */
	if ( setupLCQPdata( _H,_g,_A,_lb,_ub,_lbA,_ubA,_C ) != SUCCESSFUL_RETURN )
		return THROWERROR( RET_INVALID_ARGUMENTS );

	/* 3) Call to main initialisation routine. */
	return solveInitialLCQP( xOpt,yOpt,guessedBounds,guessedConstraints,_R, nWSR,cputime );
}
 ...
 *	i n i t
 */
returnValue LCQProblem::init(	const real_t* const _H, const real_t* const _g, const real_t* const _A,
								const real_t* const _lb, const real_t* const _ub,
								const real_t* const _lbA, const real_t* const _ubA,
 ...
 *	i n i t
 */
returnValue LCQProblem::init(	const char* const H_file, const char* const g_file, const char* const A_file,
								...
								)
{
	...
	if ( setupLCQPdataFromFile( H_file,g_file,A_file,lb_file,ub_file,lbA_file,ubA_file,C_file ) != SUCCESSFUL_RETURN )
		return THROWERROR( RET_UNABLE_TO_READ_FILE );

	if ( R_file == 0 )
	{
		/* 3) Call to main initialisation routine. */
		return solveInitialLCQP( xOpt,yOpt,guessedBounds,guessedConstraints,0, nWSR,cputime );
	}
	else
	{
		...
		return solveInitialLCQP( xOpt,yOpt,guessedBounds,guessedConstraints,R, nWSR,cputime );
	}
}
 ...
 *	h o t s t a r t
 */
returnValue LCQProblem::hotstart(	const real_t* const g_new,
 ...
 *	h o t s t a r t
 */
returnValue LCQProblem::hotstart(	const char* const g_file,
								const char* const lb_file, const char* const ub_file,
								const char* const lbA_file, const char* const ubA_file,
								int_t& nWSR, real_t* const cputime,
								const Bounds* const guessedBounds, const Constraints* const guessedConstraints
 ...
 */
returnValue LCQProblem::solveCurrentEQP(	const int_t n_rhs,
										const real_t* g_in,
										const real_t* lb_in,
										const real_t* ub_in,
										const real_t* lbA_in,
 ...
 */
returnValue LCQProblem::getWorkingSet( real_t* workingSet )
{
	int_t nV = this->getNV();
 ...
 */
returnValue LCQProblem::getWorkingSetBounds( real_t* workingSetB )
{
	return LCQProblemB::getWorkingSetBounds( workingSetB );
}
 ...
 */
returnValue LCQProblem::getWorkingSetConstraints( real_t* workingSetC )
{
	int_t i;
	int_t nC = this->getNC();
 ...
 */
returnValue LCQProblem::setConstraintProduct( ConstraintProduct* const _constraintProduct )
{
 ...
 */
returnValue LCQProblem::printProperties( )
{
...
returnValue LCQProblem::getFreeVariablesFlags( BooleanType* varIsFree )
{
	int_t nV  = getNV( );
 ...
 *	c o p y
 */
returnValue LCQProblem::copy(	const LCQProblem& rhs
							)
{
 ...
 */
returnValue LCQProblem::solveInitialLCQP(	const real_t* const xOpt, const real_t* const yOpt,
											const Bounds* const guessedBounds, const Constraints* const guessedConstraints,
											const real_t* const _R,
											int_t& nWSR, real_t* const cputime
											)
 ...
 *	s o l v e Q P
 */
returnValue LCQProblem::solveLCQP(	const real_t* const g_new,
									const real_t* const lb_new, const real_t* const ub_new,
									const real_t* const lbA_new, const real_t* const ubA_new,
...
*/
	...
	for( iter=nWSRperformed; iter<nWSR; ++iter )
	{
		...
		returnvalue = determineDataShift(	g_update,lbA_new,ubA_new,lb_new,ub_new,
		...
		returnvalue = performStep(	delta_g, delta_lbA,delta_ubA,delta_lb,delta_ub,
									delta_xFX,delta_xFR,delta_yAC,delta_yFX, penVal, g_new,
		...
		homotopyLength = getRelativeHomotopyLength( g_update,lb_new,ub_new,lbA_new,ubA_new );

		if ( homotopyLength <= options.terminationTolerance )
		{
			// Get NLP stationarity value
			nlpStationarityValue = getNorm(getNLPStationarity(x, penVal, g_new), nV);
			...
			if (nlpStationarityValue <= options.terminationTolerance) {
				...
				if (complementarityValue <= options.complementarityTolerance)
				{
					...
					if ( printIteration( iter,BC_idx,BC_status,BC_isBound,homotopyLength,penVal,isFirstCall ) != SUCCESSFUL_RETURN )
		...
		// Give some iteration status on gradient update
		if ( printIteration( iter,BC_idx,BC_status,BC_isBound,homotopyLength,penVal,isFirstCall ) != SUCCESSFUL_RETURN )
		THROWERROR( RET_PRINT_ITERATION_FAILED ); /* do not pass this as return value! */
	}
 ...
 */
returnValue LCQProblem::solveRegularisedQP(	const real_t* const g_new,
											const real_t* const lb_new, const real_t* const ub_new,
											const real_t* const lbA_new, const real_t* const ubA_new,
											int_t& nWSR, real_t* const cputime, int_t nWSRperformed,
											BooleanType isFirstCall
											)
{
	...
	if ( usingRegularisation( ) == BT_FALSE )
		return solveLCQP( g_new,lb_new,ub_new,lbA_new,ubA_new, nWSR,cputime,nWSRperformed,isFirstCall );
	...
	if ( cputime == 0 )
	{
		returnvalue = solveLCQP( g_new,lb_new,ub_new,lbA_new,ubA_new, nWSR,0,nWSRperformed,isFirstCall );
	}
	else
	{
		cputime_cur = *cputime;
		returnvalue = solveLCQP( g_new,lb_new,ub_new,lbA_new,ubA_new, nWSR,&cputime_cur,nWSRperformed,isFirstCall );
	}
	nWSR_total     = nWSR;
	...
	for( step=0; step<options.numRegularisationSteps; ++step )
	{
		...
		if ( cputime == 0 )
		{
			returnvalue = solveLCQP( gMod,lb_new,ub_new,lbA_new,ubA_new, nWSR,0,nWSR_total,isFirstCall );
		}
		else
		{
			cputime_cur = *cputime - cputime_total;
			returnvalue = solveLCQP( gMod,lb_new,ub_new,lbA_new,ubA_new, nWSR,&cputime_cur,nWSR_total,isFirstCall );
		}
 ...
 */
returnValue LCQProblem::updateActivitiesForHotstart( const real_t* const lb_new, const real_t* const ub_new,
												   const real_t* const lbA_new, const real_t* const ubA_new
												   )
{
	int_t i;
	...
	if ( LCQProblemB::setupSubjectToType( lb_new,ub_new ) != SUCCESSFUL_RETURN )
 ...
 *	s e t u p S u b j e c t T o T y p e
 */
returnValue LCQProblem::setupSubjectToType( )
{
 ...
 *	s e t u p S u b j e c t T o T y p e
 */
returnValue LCQProblem::setupSubjectToType(	const real_t* const lb_new, const real_t* const ub_new,
											const real_t* const lbA_new, const real_t* const ubA_new
											)
{
	int_t i;
 ...
 */
returnValue LCQProblem::computeProjectedCholesky( )
{
	int_t i, j;
	int_t nV  = getNV( );
	int_t nZ  = getNZ( );
	...
	if ( getNFX() + getNAC() == 0 )
		return LCQProblemB::computeCholesky( );
 ...
 */
returnValue LCQProblem::setupInitialCholesky( )
{
 ...
 */
returnValue LCQProblem::setupTQfactorisation( )
{
	int_t i, ii;
	int_t nV  = getNV( );
	int_t nFR = getNFR( );
 ...
 */
returnValue LCQProblem::obtainAuxiliaryWorkingSet(	const real_t* const xOpt, const real_t* const yOpt,
													const Bounds* const guessedBounds, const Constraints* const guessedConstraints,
													...
													) const
{
	...
	if ( LCQProblemB::obtainAuxiliaryWorkingSet( xOpt,yOpt,guessedBounds, auxiliaryBounds ) != SUCCESSFUL_RETURN )
 ...
 */
returnValue LCQProblem::setupAuxiliaryWorkingSet(	const Bounds* const auxiliaryBounds,
 ...
 */
returnValue LCQProblem::setupAuxiliaryQPsolution(	const real_t* const xOpt, const real_t* const yOpt
												)
{
	int_t i, j;
	int_t nV = getNV( );
 ...
 */
returnValue LCQProblem::setupAuxiliaryQPgradient( )
{
	int_t i;
	int_t nV = getNV( );
	int_t nC = getNC( );
 ...
 */
returnValue LCQProblem::areBoundsConsistent(	const real_t* const lb_new, const real_t* const ub_new,
											const real_t* const lbA_new, const real_t* const ubA_new) const
{
	if (LCQProblemB::areBoundsConsistent(lb_new, ub_new) == RET_QP_INFEASIBLE)
 ...
 */
returnValue LCQProblem::setupAuxiliaryQPbounds(	const Bounds* const auxiliaryBounds,
												const Constraints* const auxiliaryConstraints,
 ...
 */
returnValue LCQProblem::addConstraint(	int_t number, SubjectToStatus C_status,
										BooleanType updateCholesky,
 ...
 */
returnValue LCQProblem::addConstraint_checkLI( int_t number )
{
 ...
 */
returnValue LCQProblem::addConstraint_ensureLI( int_t number, SubjectToStatus C_status )
{
	int_t i, j, ii, jj;
	int_t nV  = getNV( );
	int_t nFR = getNFR( );
 ...
 *	a d d B o u n d
 */
returnValue LCQProblem::addBound(	int_t number, SubjectToStatus B_status,
								BooleanType updateCholesky,
								BooleanType ensureLI
								)
{
 ...
 */
returnValue LCQProblem::addBound_checkLI( int_t number )
{
	int_t i, ii;
 ...
 */
returnValue LCQProblem::addBound_ensureLI( int_t number, SubjectToStatus B_status )
{
	int_t i, ii;
	int_t nV  = getNV( );
	int_t nFX = getNFX( );
 ...
 */
returnValue LCQProblem::removeConstraint(	int_t number,
										BooleanType updateCholesky,
 ...
 */
returnValue LCQProblem::removeBound(	int_t number,
									BooleanType updateCholesky,
									BooleanType allowFlipping,
									BooleanType ensureNZC
									)
...
returnValue LCQProblem::performPlainRatioTest(	int_t nIdx,
...
returnValue LCQProblem::ensureNonzeroCurvature(	BooleanType removeBoundNotConstraint,
												int_t remIdx,
 ...
 *	b a c k s o l v e T
 */
returnValue LCQProblem::backsolveT( const real_t* const b, BooleanType transposed, real_t* const a ) const
{
	int_t i, j;
	int_t nT = getNAC( );
	int_t tcol = sizeT - nT;
 ...
 */
returnValue LCQProblem::determineDataShift(	const real_t* const g_new, const real_t* const lbA_new, const real_t* const ubA_new,
											const real_t* const lb_new, const real_t* const ub_new,
											...
											)
{
	...
	LCQProblemB::determineDataShift(	g_new,lb_new,ub_new,
 ...
 */
returnValue LCQProblem::determineStepDirection(	const real_t* const delta_g, const real_t* const delta_lbA, const real_t* const delta_ubA,
 ...
 */
returnValue LCQProblem::performStep(	const real_t* const delta_g,
 ...
 */
returnValue LCQProblem::changeActiveSet( int_t BC_idx, SubjectToStatus BC_status, BooleanType BC_isBound )
{
	int_t nV = getNV( );
 ...
 */
real_t LCQProblem::getRelativeHomotopyLength(	const real_t* const g_new, const real_t* const lb_new, const real_t* const ub_new,
												const real_t* const lbA_new, const real_t* const ubA_new
												)
{
	int_t i;
	int_t nC = getNC( );
	real_t len = LCQProblemB::getRelativeHomotopyLength( g_new,lb_new,ub_new );
	real_t d, s;
 ...
 */
returnValue LCQProblem::performRamping( )
{
 ...
 */
returnValue LCQProblem::updateFarBounds(	real_t curFarBound, int_t nRamp,
										...
										) const
{
    ...
    returnValue returnvalue = LCQProblemB::updateFarBounds(	curFarBound,nRamp,
															lb_new,lb_new_far, ub_new,ub_new_far
															);
    if ( returnvalue != SUCCESSFUL_RETURN )
        return returnvalue;
 ...
 */
returnValue LCQProblem::performDriftCorrection( )
{
	int_t i;
	int_t nV = getNV ();
	int_t nC = getNC ();
 ...
 */
returnValue LCQProblem::setupAuxiliaryQP( const Bounds* const guessedBounds, const Constraints* const guessedConstraints )
{
	int_t i, j;
	int_t nV = getNV( );
	int_t nC = getNC( );
...
BooleanType LCQProblem::shallRefactorise(	const Bounds* const guessedBounds,
										...
										)
{
...
#ifdef __WRITE_DATA_FILES__
	{
		...
		char buf[256];
		snprintf(buf,256,"QP%d_setupLCQPdata.dat",GlobalOutputFileCounter);
	...
	if ( LCQProblemB::setupLCQPdata( _H,_g,_lb,_ub,_C ) != SUCCESSFUL_RETURN )
		return THROWERROR( RET_INVALID_ARGUMENTS );
 ...
 */
returnValue LCQProblem::setupLCQPdataFromFile(	const char* const H_file, const char* const g_file, const char* const A_file,
												const char* const lb_file, const char* const ub_file,
												const char* const lbA_file, const char* const ubA_file,
												const char* const C_file
												)
{
	...
	returnvalue = LCQProblemB::setupLCQPdataFromFile( H_file,g_file,lb_file,ub_file,C_file);
	if ( returnvalue != SUCCESSFUL_RETURN )
		return THROWERROR( returnvalue );
 ...
 */
returnValue LCQProblem::loadQPvectorsFromFile(	const char* const g_file, const char* const lb_file, const char* const ub_file,
												const char* const lbA_file, const char* const ubA_file,
												...
												) const
{
	...
	returnvalue = LCQProblemB::loadQPvectorsFromFile( g_file,lb_file,ub_file, g_new,lb_new,ub_new );
	if ( returnvalue != SUCCESSFUL_RETURN )
		return THROWERROR( returnvalue );
 ...
 */
returnValue LCQProblem::printIteration( int_t iter,
										int_t BC_idx,
										SubjectToStatus BC_status,
										BooleanType BC_isBound,
										real_t homotopyLength,
		  								...
		  								)
{
	...
	switch ( options.printLevel )
	{
		...
		case PL_MEDIUM:
			...
			{
				if (comp_status == COMP_GROWING)
					snprintf( info,17,"INC PEN %1.2e", penVal );
				else if (comp_status == GRADIENT_UPDATED)
					snprintf( info,17," GRADIENT UPDATE");
 ...
 */
returnValue LCQProblem::dropInfeasibles( int_t BC_number, SubjectToStatus BC_status, BooleanType BC_isBound,
 ...
 */
real_t* LCQProblem::getNLPStationarity(	const real_t* const x_eval,
										const real_t penVal,
										const real_t* const g_original)
{
	...
	real_t* y_constraints = new real_t[nC];
	for (int i = 0; i < nC; i++) 
		y_constraints[i] = y[nV + i];

	// Compute A'*y_constraints and store in Ay.
	real_t* Ay = new real_t[nV];
	A->transTimes(1, 1.0, y_constraints, nC, 0.0, Ay, nV);

	// subtract bound constraints
	for (int i = 0; i < nV; i++){
		stat[i] -= y[i] + Ay[i];
	}
 ...
 */
returnValue LCQProblem::writeQpDataIntoMatFile(	const char* const filename
												) const
{
	#ifndef __SUPPRESSANYOUTPUT__
 ...
 */
returnValue LCQProblem::writeQpWorkspaceIntoMatFile(	const char* const filename
													)
{
	#ifndef __SUPPRESSANYOUTPUT__
